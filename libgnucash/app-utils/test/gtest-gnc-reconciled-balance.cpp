...
/* Records live in the state file, which is a process-wide singleton and
 * so outlives any one book. Each test starts from an empty one. */
static void
clear_state_file ()
{
    gnc_reconciled_balance_reset_cache ();

    auto keyfile = gnc_state_get_current ();
    gsize n_groups = 0;
    auto groups = g_key_file_get_groups (keyfile, &n_groups);

    for (gsize i = 0; groups && i < n_groups; ++i)
        g_key_file_remove_group (keyfile, groups[i], nullptr);

    g_strfreev (groups);
}

/* The store answers "is this one broken?" and nothing more; counting and
 * filtering are the caller's job -- in the shipped code, the CLI's. */
static bool
holds (const GncReconciledBalance *rb)
{
    return !gnc_reconciled_balance_is_broken (rb);
}

static guint
count_broken_for_account (const Account *acc)
{
    auto records = gnc_reconciled_balance_get_for_account (acc);
    guint n = 0;

    for (auto node = records; node; node = node->next)
        if (gnc_reconciled_balance_is_broken
            (static_cast<GncReconciledBalance*> (node->data)))
            ++n;
...
static guint
count_broken (const QofBook *book)
{
    auto records = gnc_reconciled_balance_get_all (book);
    guint n = 0;
...
protected:
    ...
    GncReconciledBalance *make_record (Account *acc, time64 date,
                                       gnc_numeric amount)
    {
        return gnc_reconciled_balance_record (acc, date, amount, nullptr);
    }
...
{
    auto ba = gnc_reconciled_balance_record (m_bank, JAN_15, dollars (50),
                                             "statement 3");
    ...
    EXPECT_STREQ ("", gnc_reconciled_balance_get_notes
                  (make_record (m_bank, FEB_15, dollars (50))));
...
/* The store is a cache over the state file; dropping it must lose
 * nothing, because that is exactly what reopening the book does. */
TEST_F (ReconciledBalanceTest, RecordsSurviveTheCacheBeingDropped)
{
    make_record (m_bank, JAN_15, gnc_numeric_create (4555, 100));
    gnc_reconciled_balance_record (m_bank, FEB_15, dollars (75),
                                   "statement 3; with a semicolon");

    gnc_reconciled_balance_reset_cache ();

    GList *back = gnc_reconciled_balance_get_for_account (m_bank);
    ASSERT_EQ (2u, g_list_length (back));

    auto first = static_cast<GncReconciledBalance*> (back->data);
    auto second = static_cast<GncReconciledBalance*> (back->next->data);

    /* Read back exactly, not as a rounded double. */
    EXPECT_TRUE (gnc_numeric_eq (gnc_numeric_create (4555, 100),
                                 gnc_reconciled_balance_get_amount (first)));
    EXPECT_EQ (gnc_time64_get_day_neutral (JAN_15),
               gnc_reconciled_balance_get_date (first));

    /* GKeyFile's own list separator is ';', so notes containing one have
     * to come back whole. */
    EXPECT_STREQ ("statement 3; with a semicolon",
                  gnc_reconciled_balance_get_notes (second));

    g_list_free (back);
}

/* The state file is written under the account's guid, in the form the
 * header documents -- a human can read and repair it. */
TEST_F (ReconciledBalanceTest, TheStateFileHoldsWhatTheHeaderPromises)
{
    gnc_reconciled_balance_record (m_bank, JAN_15, dollars (50), "statement 3");

    char guid_str[GUID_ENCODING_LENGTH + 1];
    guid_to_string_buff (xaccAccountGetGUID (m_bank), guid_str);

    auto keyfile = gnc_state_get_current ();
    ASSERT_TRUE (g_key_file_has_group (keyfile, guid_str));

    gsize n_fields = 0;
    auto fields = g_key_file_get_string_list (keyfile, guid_str, "2023-01-15",
                                              &n_fields, nullptr);
    ASSERT_NE (nullptr, fields);
    ASSERT_EQ (2u, n_fields);
    EXPECT_STREQ ("5000/100", fields[0]);
    EXPECT_STREQ ("statement 3", fields[1]);
    g_strfreev (fields);
}

/* Re-recording the same day replaces the record rather than leaving two
 * disagreeing rows for one date. */
TEST_F (ReconciledBalanceTest, RecordingTwiceForOneDateReplaces)
{
    make_record (m_bank, JAN_15, dollars (50));
    gnc_reconciled_balance_record (m_bank, JAN_15 + 3600 * 5, dollars (60),
                                   "later that day");

    GList *all = gnc_reconciled_balance_get_for_account (m_bank);
    ASSERT_EQ (1u, g_list_length (all));

    auto rb = static_cast<GncReconciledBalance*> (all->data);
    EXPECT_TRUE (gnc_numeric_equal (dollars (60),
                                    gnc_reconciled_balance_get_amount (rb)));
    EXPECT_STREQ ("later that day", gnc_reconciled_balance_get_notes (rb));
...
TEST_F (ReconciledBalanceTest, MismatchedBalanceIsBroken)
{
    ...
    EXPECT_TRUE (gnc_reconciled_balance_is_broken (rb));
...
TEST_F (ReconciledBalanceTest, ALateEntryBreaksEverySealOnOrAfterIt)
{
    ...
    ASSERT_TRUE (holds (jan));
    ASSERT_TRUE (holds (feb));

    /* The cheque was written on the 15th of January and entered now. */
    add_transaction (JAN_15, dollars (-20));

    EXPECT_TRUE (gnc_reconciled_balance_is_broken (jan));
    EXPECT_TRUE (gnc_reconciled_balance_is_broken (feb));
...
/* Accepting a change is just recording the date again: the same date
 * replaces rather than accumulates, so the record moves to what the book
 * now holds. This is what a second reconciliation of a statement does. */
TEST_F (ReconciledBalanceTest, RecordingTheDateAgainAcceptsTheChange)
{
    add_transaction (JAN_15, dollars (100));
    auto rb = make_record (m_bank, JAN_15, dollars (100));

    add_transaction (JAN_15, dollars (-20));
    ASSERT_TRUE (gnc_reconciled_balance_is_broken (rb));

    rb = gnc_reconciled_balance_record
        (m_bank, JAN_15, gnc_reconciled_balance_compute (m_bank, JAN_15),
         "accepted");
    ...
    GList *records = gnc_reconciled_balance_get_for_account (m_bank);
    EXPECT_EQ (1u, g_list_length (records));
    g_list_free (records);
}
...
TEST_F (ReconciledBalanceTest, ListsAreSortedByDateAndFilteredByAccount)
{
    ...
    EXPECT_EQ (gnc_time64_get_day_start (JAN_15),
               gnc_time64_get_day_start (gnc_reconciled_balance_get_date
                                         (static_cast<GncReconciledBalance*> (bank->data))));
    g_list_free (bank);
}
...
TEST_F (ReconciledBalanceTest, CountsOnlyBrokenRecords)
{
    ...
    EXPECT_EQ (1u, count_broken (m_book));
    EXPECT_EQ (1u, count_broken_for_account (m_bank));
    EXPECT_EQ (0u, count_broken_for_account (m_income));

    EXPECT_EQ (0u, count_broken_for_account (m_income));
}

TEST_F (ReconciledBalanceTest, RemoveTakesTheRecordOutOfTheStateFile)
{
    auto rb = make_record (m_bank, JAN_15, dollars (50));
    ...
    gnc_reconciled_balance_remove (rb);

    EXPECT_EQ (nullptr, gnc_reconciled_balance_get_all (m_book));
    EXPECT_FALSE (g_key_file_has_group (gnc_state_get_current (), guid_str));
}
...
TEST_F (ReconciledBalanceTest, DeletingTheAccountRemovesItsRecords)
{
    ...
    if (remaining)
    {
        EXPECT_EQ (m_income, gnc_reconciled_balance_get_account
                   (static_cast<GncReconciledBalance*> (remaining->data)));
    }
...
protected:
    ...
    {
        return gnc_reconciled_balance_record
            (acc, date, gnc_reconciled_balance_compute (acc, date), nullptr);
    }
...
TEST_F (ReconciledBalanceBookTest, TheBookStartsSound)
{
    ...
    EXPECT_EQ (0u, count_broken (m_book));
}
...
TEST_F (ReconciledBalanceBookTest, DamageToOneAccountLeavesTheOthersIntact)
{
    ...
    EXPECT_EQ (3u, count_broken_for_account (m_bank));
    EXPECT_EQ (0u, count_broken_for_account (m_card));
    EXPECT_EQ (3u, count_broken (m_book));
}
...
TEST_F (ReconciledBalanceBookTest, TwoKindsOfDamageAtOnceAreBothReported)
{
    ...
    EXPECT_EQ (5u, count_broken (m_book));
}
...
/* Sealing an earlier date says nothing about the later records and must
 * not disturb them. Reconciling moves no money -- it sets flags -- so a
 * record dated after the statement being reconciled is still true, and
 * dropping or re-sealing it would throw away a live check. */
TEST_F (ReconciledBalanceBookTest, ReconcilingAnEarlierDateLeavesLaterRecordsAlone)
{
    /* Reconcile November again, as re-running that statement would, and
     * seal it again as finishing the reconciliation does. */
    reconcile (m_nov_groceries, m_bank, on (30, 11, 2025));
    seal (m_bank, on (30, 11, 2025));
...
/* The case that makes the rule worth stating: a reconcile session where
 * the user enters a transaction they had missed. That is a real change
 * to a period two later records cover, and they break -- by the same
 * amount, which is what says it was one entry. Silently re-sealing them
 * because a reconciliation had just run would destroy exactly the
 * evidence the user wants. */
TEST_F (ReconciledBalanceBookTest, AnEntryMadeWhileReconcilingStillBreaksLaterRecords)
{
    post (on (20, 11, 2025), m_bank, m_expense, 4500, "Bank charge, missed");
    seal (m_bank, on (30, 11, 2025));
    ...
    EXPECT_TRUE (gnc_numeric_equal (delta (dec (m_bank)),
                                    delta (jan (m_bank))));
}
...
{
    auto date = on (31, 1, 2026);
    auto amount = gnc_reconciled_balance_compute (m_bank, date);

    /* Four tenths of a cent above the real balance. */
    auto rb = gnc_reconciled_balance_record
        (m_bank, date, gnc_numeric_add (amount, gnc_numeric_create (4, 1000),
...
/* The late cheque was real: accept it by sealing each date again, and
 * the records stand at the balances the book now has. */
TEST_F (ReconciledBalanceBookTest, RecordingAgainAfterALateEntryRestoresEverySeal)
{
    post (on (25, 11, 2025), m_bank, m_expense, 6000, "Cheque 1234");
    ASSERT_EQ (3u, count_broken_for_account (m_bank));

    for (auto date : { on (30, 11, 2025), on (31, 12, 2025), on (31, 1, 2026) })
        seal (m_bank, date);

    EXPECT_EQ (0u, count_broken (m_book));

    /* November is now 2770 - 60. */
    EXPECT_TRUE (gnc_numeric_equal
                 (money (271000),
                  gnc_reconciled_balance_compute (m_bank, on (30, 11, 2025))));
}

/* Accepting one date says nothing about the others. */
TEST_F (ReconciledBalanceBookTest, RecordingOneDateAgainLeavesTheRestBroken)
{
    post (on (25, 11, 2025), m_bank, m_expense, 6000, "Cheque 1234");

    seal (m_bank, on (30, 11, 2025));

    EXPECT_EQ (2u, count_broken_for_account (m_bank));
}
