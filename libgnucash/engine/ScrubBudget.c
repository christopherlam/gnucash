...
static void
check_if_fixes_needed (QofInstance* data, gpointer user_data)
{
    Account *root = (Account*) user_data;

    if (heuristics_on_budget (GNC_BUDGET(data), root) != HEURISTICS_NONE)
        this_book_needs_fixing = TRUE;
}

static gboolean
check_any_broken_budgets (QofBook *book)
{
    Account* root = gnc_book_get_root_account (book);

    this_book_needs_fixing = FALSE;

    qof_collection_foreach (qof_book_get_collection (book, GNC_ID_BUDGET),
                            check_if_fixes_needed, root);

    return this_book_needs_fixing;
}

/* analyse book, and return following:
 * FALSE: don't offer scrubbing
 * TRUE:  offer scrubbing */
gboolean
gnc_scrub_budget_signs_check (QofBook *book)
{
    /* feature set, or there are no budgets. skip scrubbing. */
    if (gnc_features_check_used (book, GNC_FEATURE_BUDGET_UNREVERSED) ||
        (!gnc_budget_get_default (book)))
        return FALSE;

    /* if there are broken budgets, signal to offer scrubbing. */
    return (check_any_broken_budgets (book));
}
...
gnc_scrub_budget_signs (QofBook *book)
{
    ...
    if (gnc_features_check_used (book, GNC_FEATURE_BUDGET_UNREVERSED))
    {
        PWARN ("Don't run scrub_budget_signs_doit with featured book!");
        return;
    }
    ...
    PWARN ("Budget sign reversals complete");
}
/* ==================== END OF FILE ==================== */
