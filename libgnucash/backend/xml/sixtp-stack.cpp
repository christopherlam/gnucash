                   ...
                   gpointer top_level_data)
{
    ...
    if (initial_parser->start_handler)
    {
        /* Pass the caller-supplied top_level_data itself (typically NULL,
           ...
           Passing &ret->top_frame_data instead hands start_handler the
           address of this context's own field as a bogus non-NULL
           "parent", which a DOM-building start_handler then tries to
           read/link into as if it were a real parent node - reading a few
           pointer-widths past the end of this g_new0'd struct. */
