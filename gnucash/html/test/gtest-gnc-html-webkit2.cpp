...
INSTANTIATE_TEST_SUITE_P (Http, ExtractBaseNameTest, ::testing::Values(
    /* filename at end of a multi-segment path: the regression case ctre
     * introduced (ctre::match requires the whole string to match, so a
     * path not ending in '/' used to yield nullptr instead of the base
     * directory) */
    std::make_tuple (URL_TYPE_HTTP, "//www.gnucash.org/docs/help/index.html",
                      "//www.gnucash.org/docs/help/"),
    /* already a directory (trailing slash) is left untouched */
    std::make_tuple (URL_TYPE_HTTP, "//www.gnucash.org/docs/help/",
                      "//www.gnucash.org/docs/help/"),
    /* single path segment: filename directly under the host root */
    std::make_tuple (URL_TYPE_HTTP, "//www.gnucash.org/index.html",
                      "//www.gnucash.org/"),
    /* host root, explicit trailing slash */
    std::make_tuple (URL_TYPE_HTTP, "//www.gnucash.org/",
                      "//www.gnucash.org/"),
...
INSTANTIATE_TEST_SUITE_P (Secure, ExtractBaseNameTest, ::testing::Values(
    ...
    std::make_tuple (URL_TYPE_SECURE, "//example.com/a/b/report.html",
                      "//example.com/a/b/"),
    std::make_tuple (URL_TYPE_SECURE, "//example.com:8443/report.html",
                      "//example.com:8443/")
));

INSTANTIATE_TEST_SUITE_P (Ftp, ExtractBaseNameTest, ::testing::Values(
    std::make_tuple (URL_TYPE_FTP, "//ftp.gnu.org/gnu/gnucash/file.tar.gz",
                      "//ftp.gnu.org/gnu/gnucash/"),
...
INSTANTIATE_TEST_SUITE_P (FileAndOtherTypes, ExtractBaseNameTest, ::testing::Values(
    /* absolute filesystem path with a filename: same "not ending in '/'"
     * regression case as the http group above, exercised via the else
     * branch (re2) */
    std::make_tuple (URL_TYPE_FILE, "/usr/share/gnucash/report.html",
                      "/usr/share/gnucash/"),
    std::make_tuple (URL_TYPE_FILE, "/usr/share/gnucash/",
                      "/usr/share/gnucash/"),
    ...
    std::make_tuple (URL_TYPE_XMLDATA, "/data/accounts/foo.xml",
                      "/data/accounts/"),
    std::make_tuple (URL_TYPE_JUMP, "somewhere/on/the/page.html",
                      "somewhere/on/the/")
));
